...
router.get("/absentpayment", function (req, res) {
  ...
  mysqlConnection.query(
    "SELECT StudentId,Username,FirstName,MobileNumber,Grade,Email FROM student_information WHERE StudentId not in (SELECT DISTINCT StudentId FROM payments WHERE Month='" +
      date +
      "' )",
    function (err, result) {
      if (err) {
